...
async def test_seed_historique_valide_top_n_et_ajoute_feedback(
    ...
    await _make_proposees(db_session, sess.id, professeur_prof.id, [0.4, 0.9, 0.7, 0.5])

    result = await seed_historique_session(sess.id, test_user_rh.id, db_session, nb_cible=2, note=5)
    ...
    feedbacks = (await db_session.execute(select(AffectationFeedback))).scalars().all()
...
@pytest.mark.asyncio
async def test_seed_historique_idempotent(db_session: AsyncSession, professeur_prof, test_user_rh):
    """Un 2e appel sur la même session ne touche à rien."""
    sess = Session(annee=2031, semestre=Semestre.AUTOMNE)
    db_session.add(sess)
    await db_session.commit()
    await db_session.refresh(sess)
    await _make_proposees(db_session, sess.id, professeur_prof.id, [0.6, 0.8, 0.7])

    await seed_historique_session(sess.id, test_user_rh.id, db_session, nb_cible=2)
    result = await seed_historique_session(sess.id, test_user_rh.id, db_session, nb_cible=2)

    assert result.deja_alimente is True
    assert result.nb_validees == 0
    nb_feedbacks = (
        await db_session.execute(select(func.count(AffectationFeedback.id)))
    ).scalar_one()
    assert nb_feedbacks == 2  # toujours 2, pas de doublons


@pytest.mark.asyncio
async def test_seed_historique_aucune_proposee(db_session: AsyncSession, test_user_rh):
