...
const SEGMENTS = [
  { key: "comp" as keyof Scores, pKey: "w1" as keyof Poids, color: "#3b82f6", label: "Comp." },
  { key: "exp" as keyof Scores, pKey: "w2" as keyof Poids, color: "#22c55e", label: "Exp." },
  { key: "hist" as keyof Scores, pKey: "w3" as keyof Poids, color: "#f97316", label: "Hist." },
  { key: "sem" as keyof Scores, pKey: "w4" as keyof Poids, color: "#a855f7", label: "Sém." },
];
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex h-2 w-full overflow-hidden rounded-full bg-secondary">
        {SEGMENTS.map(({ key, pKey, color }) => {
          const contribution = scores[key] * poids[pKey];
          return (
            <div
              key={key}
              style={{ width: `${Number(scores[key]) * Number(poids[pKey]) * 100}%`, backgroundColor: color }}
              title={`${key}: ${Math.round(Number(scores[key]) * 100)}%`}
            />
          );
        })}
      </div>
