...
2. HR generates assignments for an academic session. Each teacher/course pair
   ...
   PostgreSQL `CHECK` constraint. W4 uses sentence-transformers embeddings.
...
Lead developer: architecture, backend pipeline, frontend, CI/Docker. Teammates
contributed to the project alongside me — see the team section of
[README.fr.md](README.fr.md) for the full list. The git history shows every
commit authored by me, with co-author trailers for two teammates.

<!-- TODO: préciser la répartition exacte des contributions de l'équipe -->
...
└───────────────────┘           └────────┬─────────┘
                                    ...
                                    │ worker   │             └──────────────┘
                                    └────┬─────┘
                       CV text ─────────►│◄───────── LLM (OpenAI-compatible)
                       extraction        │           embeddings (local model)
```
...
| AI | LLM through an OpenAI-compatible endpoint; `sentence-transformers` (`all-MiniLM-L6-v2`, 384 dimensions) for the semantic score; `pdfplumber` / `python-docx` for text extraction |
...
- **Lazy model loading.** `sentence-transformers` is imported inside the
  function that needs it, so API start-up does not pay the PyTorch import cost;
  the Docker image installs the **CPU** torch wheel and preloads the embedding
  model so the container works offline.
- **Deterministic embeddings in tests.** An autouse fixture replaces the model
  with a hash-based encoder: the real `compute_embedding` code path is tested
  without downloading a model or requiring a GPU.
...
| `sentence-transformers` pulling the CUDA stack (~2.8 GB) into a slim image | CPU torch wheel installed from the PyTorch CPU index before the project, and the model preloaded at build time |
...
|---|---|---|
| Backend image | `download.pytorch.org`, PyPI | CPU torch wheel |
| Backend image | `huggingface.co` | `all-MiniLM-L6-v2` is preloaded into the image; without this preload the model would instead be downloaded at the first request that computes an embedding (demo seed, course creation, assignment generation) |
...
The first build is therefore not a "3 minutes" operation: expect a multi-GB
download (torch CPU + model) on a cold cache.
...
pip install torch --index-url https://download.pytorch.org/whl/cpu  # avoids the CUDA stack
pip install -e ".[dev]"
